...
int lcsRec(string str1,string str2){
    ...
    if(str1[n-1]==str2[m-1]){//abcde->adcd    ->abc
        return 1+lcsRec(str1.substr(0,n-1),str2.substr(0,m-1));//(index,length)
    }else{
...
int lcsMem(string str1,string str2,vector<vector<int>> &dp){//O(n*m)
    ...
    if(str1[n-1]==str2[m-1]){//abcde->adcd    ->abc
        dp[n][m]=1+lcsMem(str1.substr(0,n-1),str2.substr(0,m-1),dp);//(index,length)
    }else{
